    ...
    <div class="card mb-3">
        ...
        <div class="table-responsive rounded-3">
            <table class="table table-hover">
                <thead class="table-primary"><tr><th>Id</th><th>Created at</th><th>Updated at</th><th>Patient id</th><th>Remark</th><th>Receipt total</th><th>Discount</th><th>Vat</th><th>Actions</th></tr></thead>
                <tbody>
                @foreach ($money_receipts as $item)
                    <tr><td>{{ $item->id }}</td><td>{{ $item->created_at }}</td><td>{{ $item->updated_at }}</td><td>{{ optional($item->patient)->name ?? $item->patient_id }}</td><td>{{ $item->remark }}</td><td>{{ $item->receipt_total }}</td><td>{{ $item->discount }}</td><td>{{ $item->vat }}</td><td style="min-width:220px">
