...
<div class="page-inner">
    ...
    <div class="card mb-3">
        ...
        <div class="table-responsive rounded-3">
            <table class="table table-hover">
                <thead class="table-primary"><tr><th>Id</th><th>Bed number</th><th>Ward id</th><th>Bed type</th><th>Status</th><th>Created at</th><th>Updated at</th><th class='text-center'>Actions</th></tr></thead>
                <tbody>
                @foreach ($beds as $item)
                    <tr><td>{{ $item->id }}</td><td>{{ $item->bed_number }}</td><td>{{ optional($item->ward)->name ?? $item->ward_id }}</td><td>{{ $item->bed_type }}</td><td>{{ $item->status }}</td><td>{{ $item->created_at }}</td><td>{{ $item->updated_at }}</td><td style="min-width:220px">
    ...
    <form action="{{ route('beds.destroy', $item->id) }}" method="POST" style="display:inline;">
        @csrf
        @method('DELETE')
